...
from fasthtml.common import (
    Form, Div, Span, P, Hr, Input, Button,
    Title, Main, RedirectResponse
)  # importa explicitamente componentes usados
...
def setup_auth_routes(rt):
    print("DEBUG: Configurando rotas de autenticação (versão revertida)...")

    @rt("/login", methods=["GET"])
    def get():
        print("DEBUG: Handler GET /login executado")
        frm = Form(
            Div(
                Div(
                    Span("🌲 ", cls="logo"),
    ...
    def post(login: Login, sess):
        print("DEBUG: Handler POST /login executado")
        email_norm = login.email.strip().lower() if login.email else ''
        print(f"DEBUG: Login attempt - Raw Email: {login.email} | Normalized: {email_norm}")
        user = get_user_by_credentials(email_norm, login.pwd)
